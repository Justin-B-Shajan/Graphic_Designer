...
// Projects with optional case-study slug
const projects = [
  {
    id: 1,
...
// Reusable project card — links to case study if slug exists, else opens lightbox
function ProjectCard({
  ...
  index: number;
  isVisible: boolean;
  onLightboxOpen: (image: string, title: string) => void;
}) {
  const cardInner = (
    <div className="relative aspect-[4/5] rounded-lg overflow-hidden bg-gray-100 group-hover:shadow-2xl transition-all duration-500">
      ...
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-all duration-500 flex items-center justify-center">
        {project.slug ? (
          <span className="bg-white/20 backdrop-blur-md border border-white/20 text-white px-4 py-2 rounded-full text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center gap-2">
      ...
      {/* Case study badge */}
      {project.slug && (
        <div className="absolute top-3 left-3 bg-[#B91C1C] text-white text-[10px] uppercase tracking-widest px-2 py-1 rounded-sm font-bold">
          Case Study
        </div>
      )}
    </div>
  ...
  const wrapperClass = `group cursor-pointer transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
    }`;
  ...
  return (
    <div
      className={wrapperClass}
      style={{ transitionDelay: `${index * 100}ms` }}
      onClick={() => onLightboxOpen(project.image, project.title)}
    >
      {cardInner}
      {meta}
    </div>
...
export default function RedFeaturedWorks() {
  ...
  return (
    <section id="portfolio" ref={ref} className="relative" aria-label="Portfolio">
      ...
      <div className="bg-white py-16 sm:py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-12 sm:mb-16">
            <h2
              className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-black transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
            >
              Featured Works
            </h2>
            <p className="hidden sm:block text-sm text-black/40 uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {projects.slice(0, 6).map((project, index) => (
              <ProjectCard
                ...
                index={index}
                isVisible={isVisible}
                onLightboxOpen={(image, title) => setSelectedImage({ image, title })}
              />
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 items-center">
            <div className="space-y-4 sm:space-y-6 order-2 lg:order-1">
              ...
              <Link
                href={`/case-study/${projects[3].slug}`}
                className="inline-flex items-center gap-3 px-6 py-3 bg-white text-black font-bold text-sm uppercase tracking-wide hover:bg-white/90 transition-all duration-300 hover:gap-5 rounded-sm"
              >
